...
struct WordNode
{
	...
	void AddNeighbor(WordNode* node)
	{
		neighbors.push_back(node->val);
	}
...
public:
	int ladderLength(string beginWord, string endWord, vector<string>& wordList) 
	{
		...
		map<string, WordNode> nodeDict;
		for (int i = 0; i < len; ++i)
		{
			string nodeKey = wordList[i];
			if (nodeDict.find(nodeKey) == nodeDict.end())
				nodeDict[nodeKey] = WordNode(nodeKey);
			for (int j = i + 1; j < len; ++j)
			{
				string tempKey = wordList[j];
				if (GetDifferenCnt(nodeKey, tempKey) == 1)
				{
					if (nodeDict.find(tempKey) == nodeDict.end())
						nodeDict[tempKey] = WordNode(tempKey);
					nodeDict[nodeKey].AddNeighbor(&nodeDict[tempKey]);
					nodeDict[tempKey].AddNeighbor(&nodeDict[nodeKey]);
				}
			}
		}

		deque<string> tobeSearch;
		tobeSearch.push_back(beginWord);
		int pathLength = 0;
		while(!tobeSearch.empty())
		{
			pathLength++;

			int cnt = tobeSearch.size();
			while(cnt>0)
			{
				cnt--;
				string curNodeKey = tobeSearch.front();
				auto curNode = &nodeDict[curNodeKey];
				tobeSearch.pop_front();
				if (curNode->GetVal() == endWord)
					return pathLength;

				for (auto it = curNode->neighbors.begin(); it != curNode->neighbors.end(); ++it)
				{
					auto node = nodeDict[*it];
					if (!node.visited)
						tobeSearch.push_back(node.val);
				}
			}
		}
		return 0;
	...
	int GetDifferenCnt(string a, string b)
	{
		int ret = 0;
	...
	void RunTest()
	{
		...
		/*RunTestCase("ab", "cd", { "ad", "cd", "ae" , "af", "cb","db" });*/
